...
console.log(dept);
    ...
    html += `
           <div class="result-box">
               ...
               <div>Departments list: ${joined}</div>
            </div>
    `;
...
const joined = uniqueSkills.join(", ");
    const grid = document.getElementById('employeesGrid');
    const filter = document.getElementById('filterInfo');
    let html = '';
    let infoHTML = '';
    ...
    infoHTML += `
        <div><strong>Showing:</strong> Bonus 3: Unique skills - ${joined}</div>
    `;

    html += `
           <div class="result-box">
               <h2>Bonus 3</h2>
               <p>Unique skills: ${joined}</p>
            </div>
    `;
